    ...
    return (
        <div className='absolute top-0 bg-primary-black h-full w-full flex justify-center items-center bg-opacity-30'>
            <div className='flex flex-col w-2/6 p-5 h-fit rounded-lg bg-gray-800'>
                ...
                <button className={`${username == '' ? 'bg-opacity-25' : ""} bg-yellow-500 text-black font-plex-mono px-4 w-fit py-3 rounded-md shadow-lg shadow-slate-900 mx-auto mt-3 active:shadow-none text-lg`}
                    onClick={() => navigate(`/problemstatements`)}>{username == "" ? "Enter a username" : "Start Contest"}</button>
            </div>
        </div>
    )
}
