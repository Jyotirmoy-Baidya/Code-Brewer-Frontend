const ContestBlock = ({ contest, setContestCode, setUsernamePopUp }) => {
    return (
        <div className="h-20 bg-primary-black flex items-center justify-between rounded-md px-4 py-4   shadow-primary">
            ...
            <button className={`${contest.active?'bg-primary':'bg-red-700'} rounded font-plex-mono text-black px-4 py-2`} onClick={() => { setContestCode(contest.contestCode); setUsernamePopUp(true); }}>Enter</button>
        </div>
    )
}
